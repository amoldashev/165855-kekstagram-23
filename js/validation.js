  ...
  hashtags.forEach((hash) => {
    if (!SingleHashExp.test(hash)) {
      input.setCustomValidity(
        'Cтрока после решётки должна состоять из букв и чисел и не может содержать пробелы, спецсимволы, символы пунктуации и эмодзи',
      );
      ...
      input.setCustomValidity(
        `Удалите ещё ${hash.length - MAX_HASHTAG_LENGTH} симв.`,
      );
    } else {
      return hashtags;
    }
...
const validateInputHashtag = (input) => {
  if (!isHashtagFirst(input)) {
    ...
    input.setCustomValidity(
      `Минимум ещё ${MIN_HASHTAG_LENGTH - getValueLength(input)} симв.`,
    );
  } else if (!whiteSpace.test(input.value) === false) {
    input.setCustomValidity(
      'Вводите хэш-теги, разделяя каждый хеш-тег пробелом',
    );
...
const validateComments = (input) => {
  if (getValueLength(input) < MAX_SYMBOL_LENGTH) {
    input.setCustomValidity(
      `Текст не должен превышать 140 симв. Осталось ${
        MAX_SYMBOL_LENGTH - getValueLength(input)
      } симв.`,
    );
  } else if (getValueLength(input) >= MAX_SYMBOL_LENGTH) {
    input.setCustomValidity(
      `Текст не должен превышать ${MAX_SYMBOL_LENGTH} симв.`,
    );
  } else {
    input.setCustomValidity('');
  }
...
inputTextDescription.addEventListener('input', () => {
